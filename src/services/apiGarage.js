...
function normalizeGarageWeekPlanning(data) {
    ...
    return {
        jours: planningItems.map((item, index) => ({
            jourId: item?.id_jour || item?.jourId || index + 1,
            libJour: item?.lib_jour || item?.libJour || normalizeDayLabel(item?.lib_jour || item?.libJour),
            isActive: Boolean(
                item?.horaire ||
                item?.id_horaire ||
            ...
            horaires: {
                semaine1: {
                    idHoraire: item?.horaire?.id_horaire || item?.id_horaire || null,
...
function normalizeGarageRdvCollection(data) {
    const rdv = Array.isArray(data)
        ? data
    ...
    return {
        ...
        rdv: rdv.map((item) => ({
            ...item,
            idRdv: item?.idRdv || item?.id_rdv || item?.id || null,
            dateDebut: item?.dateDebut || item?.date_debut || null,
            dateFin: item?.dateFin || item?.date_fin || null,
            status:
                item?.status?.libStatusRdv ||
                item?.status?.lib_status_rdv ||
                item?.status ||
                item?.statusLabel ||
                "",
        })),
...
export async function getClientRendezVous(token, clientId) {
    if (!clientId) {
        throw new Error("clientId requis pour recuperer les rendez-vous client")
    }

    return apiFetch(`/api/v1/client/${clientId}/rdvs`, {
        method: "GET",
        headers: buildAuthHeaders(token),
    })
}
...
export async function getClientDashboard(token) {
    ...
    const clientId = profile?.clientId || profile?.idClient
    const rdvRaw = clientId ? await getClientRendezVous(token, clientId) : []
...
export async function createOrUpdateGarageHoraire(token, data) {
    return apiFetch("/api/v1/horaires/create", {
        method: "POST",
        headers: buildAuthHeaders(token),
...
export async function updateGarageHoraires(token, data) {
    return apiFetch("/api/v1/horaires/ouvertures_fermetures", {
        method: "PATCH",
        headers: buildAuthHeaders(token),
        body: JSON.stringify(data),
    })
...
export async function getGaragePlanningSemaine(token, { garageId, userId } = {}) {
    ...
    const data = await apiFetch(`/api/v1/planning/semaine${suffix}`, {
        method: "GET",
        headers: buildJsonHeaders(token),
    })

    return normalizeGarageWeekPlanning(data)
}
...
export async function getGarageRdvList(token, garageId, { userId } = {}) {
    ...
    if (userId) params.set("userId", userId)
    const suffix = params.toString() ? `?${params.toString()}` : ""
    const data = await apiFetch(`/api/v1/rdv${suffix}`, {
        method: "GET",
        headers: buildJsonHeaders(token),
    })

    return normalizeGarageRdvCollection(data)
}
