...
export function getStoredAuth() {
    ...
    // Fallback sur le systeme AuthConnexion existant
    const legacyToken = token || localStorage.getItem("token") || sessionStorage.getItem("token")
    const legacyUser = profile || parseStoredJson(localStorage.getItem("user")) || parseStoredJson(sessionStorage.getItem("user"))

    return {
        token: legacyToken,
        role,
        profile: legacyUser,
        garageId: getGarageIdFromProfile(legacyUser),
        clientId: getClientIdFromProfile(legacyUser),
    }
}
...
export function setStoredAuth({ token, role, remember, profile = null }) {
    ...
    // Synchronise avec le systeme AuthConnexion existant
    targetStorage.setItem("token", token)
    if (profile) {
        targetStorage.setItem("user", JSON.stringify(profile))
    } else {
...
export async function loginUser({ email, password, otp = "", remember }) {
    ...
    // BYPASS 2FA: Essayer de desactiver le 2FA avant le login (pour le developpement)
    try {
        await fetch(`${API_BASE_URL}/api/v1/users/desactiver_2fa`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ email, mdp: password }),
        })
    } catch {
        // Ignorer l'erreur, on continue avec le login normal
    }
    ...
    for (const path of loginPaths) {
        try {
            ...
            // BYPASS 2FA: Si requires2fa est retourne mais qu'on a un token, on continue quand meme
            if ((response.status === 202 && data?.requires2fa) || data?.requires2fa) {
                const token = data?.token || data?.access_token || data?.jwt
                if (token) {
                    // Token present malgre requires2fa, on l'utilise (bypass)
                    break
                }
                // Sinon, on continue avec la reponse actuelle (qui echouera mais montrera l'erreur)
            }
...
export async function registerClient({
    ...
    if (!isValidPhoneFormat(tel)) {
        throw new Error("Merci de saisir un numero de telephone valide.")
    }
...
export async function getGaragesByVilleAndPrestation(villeId, prestationId) {
    return apiFetch(`/api/v1/get_garages_by_ville_and_prestation/${villeId}/${prestationId}`, {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
        },
...
export async function getClientDashboard(token) {
    ...
    // Recupere le profil utilisateur connecte
    const profileResult = await fetchConnectedUser(token)

    // Recupere les rendez-vous du client
...
export async function getPropositionsByGarage(garageId, token = "") {
...
// --- 2FA USERS ---
export async function activer2FA(token, email) {
...
export async function desactiver2FA(token, email) {
